...
// npx unpacks the package into a per-invocation cache directory (e.g. `npm-cache/_npx/<hash>` on
// Windows, `~/.npm/_npx/<hash>` elsewhere). `packageRoot` there is real but not durable: clearing
// the npm cache or bumping the pinned version moves it, silently breaking any mcp.json generated
// from it. Detect that case so callers can tell the user to install first.
export function looksLikeEphemeralNpxCache(root) {
...
export async function installQoder(directory, { force = false, root = packageRoot, node = process.execPath } = {}) {
  ...
  const ephemeralNpxCache = looksLikeEphemeralNpxCache(root);
  ...
  return {
    ...
    ephemeral_npx_cache: ephemeralNpxCache,
    next: [
      ...(ephemeralNpxCache
        ? [
            `Warning: this was generated by npx, so .qoder/mcp.json points into a temporary npx cache (${root}). ` +
              "Clearing the npm cache or bumping the pinned version moves that path and breaks the server silently. " +
              "Run `npm install -g chilon-recall@<version>` (or use a source checkout) and re-run `chilon-recall qoder` " +
              "from that install so the generated path is durable."
          ]
        : []),
