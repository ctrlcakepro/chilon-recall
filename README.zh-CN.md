...
[快速开始](#快速开始) · [能做什么](#能做什么) · [连接客户端](#连接-mcp-客户端) · [安全策略](SECURITY.md) · [参与贡献](CONTRIBUTING.md)
...
- [快速开始](#快速开始)
- [为什么使用 Chilon Recall？](#为什么使用-chilon-recall)
- [能做什么](#能做什么)
- [详细安装与配置](#详细安装与配置)
...
- 托管 engine 保存在临时 npx cache 之外。可用 `CHILON_RECALL_HOME` 指定其他持久位置。
- 升级 package 后请再次运行 `setup`。
- 安装过程不会写入任何凭据。请在安装完成后自行在环境变量中设置 provider key。
...
> 通过 npm 安装时，`init` 已在资料目录写入 `chilon-recall.json`，并把 `project_dir` 设为 `.`、`rag_dir` 设为 `./.chilon-recall`。编辑其中的 provider 字段即可，无需执行本小节。
...
| [DeepSeek Harness](#deepseek-harness) | `dsh --patch` overlay 或已安装的 profile bundle |
| [Claude Desktop](#claude-desktop) | `claude_desktop_config.json` |
| [Qoder](#qoder) | **Qoder IDE Settings → MCP → My Servers**，外加生成的 `.qoder/` 目录 |
...
**一次性运行** —— 无需安装 bundle，直接使用 overlay：
...
**持久 profile** —— 先安装一次仓库 bundle，再启动 profile。Windows 当前 DSH/pnpm 的路径转发可能拆分含空格的源码路径，必要时请使用 8.3 短路径：
...
- `rag_build`、`rag_sync`、`rag_clear_index`、`rag_restore_index` 必须先使用 `action: "preview"`。预览会返回与当前配置、来源和索引状态绑定的短期 token，再用 `action: "execute"` 执行一次。
- `rag_sync` 通过文件哈希复用兼容的未变更向量，并同步新增、修改和删除；索引设置变化或旧 manifest 缺少必需哈希时会自动回退到全量重建。

## Provider 配置

### 向量化

首版 embedding 支持 `openai-compatible` adapter，需要配置 `base_url`、`model`、`api_key_env`，也可配置 `doc_prefix` 与 `query_prefix`。
...
> “兼容接口”只是接口层面的声明，并不保证所有 provider 行为完全相同。索引私有资料或产生大量费用前，应先用合成示例验证自选模型。
...
测试只使用合成资料与 mock provider，不需要付费 API key。参阅 [CONTRIBUTING.md](CONTRIBUTING.md) 和 [SECURITY.md](SECURITY.md)。
