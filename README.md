...
After the npm release is published, install the isolated Python engine and create a private configuration with:

npm 版本发布后，可用以下命令安装独立 Python engine 并创建私有配置：
...
For an npm release, use a pinned npx command instead. Run npx -y chilon-recall@0.1.0 setup first under the same OS account. A pinned version prevents an unexpected package upgrade from changing a working MCP server.
